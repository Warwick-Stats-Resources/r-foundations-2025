...
```{r}
#| dpi: 120
with(penguins, boxplot(body_mass ~ species))
```
:::
...
```{r}
#| dpi: 120
plot(density(penguins$body_mass, na.rm = TRUE))
```
:::
...
```{r}
ggplot(penguins) +
  geom_boxplot(aes(species, body_mass))
```
...
A big update to ggplot2 was released in September 2025. 

The [announcement blog post](https://tidyverse.org/blog/2025/09/ggplot2-4-0-0/) 
and [styling blog post](https://tidyverse.org/blog/2025/10/ggplot2-styling/) contain details of the changes.

## Ink and paper in ggplot2 4.0.0

::: panel-tabset
### plot

![]( `r knitr::fig_chunk("ink-paper", "png") `){width="80%"}
...
# University of Warwick brand colours
purple <- "#440379"
coral <- "#F1977F"
pale_yellow <- "#FFF8E4"
...
ggplot(penguins, aes(bill_len, bill_dep)) +
  ...
  theme_minimal(paper = pale_yellow, ink = purple, accent = coral)
```
...
Also, [TidyTuesday](https://github.com/rfordatascience/tidytuesday) (#TidyTuesday on Mastodon and BlueSky) is a great source for further inspiration and practice.

## ggplot2 resources {.smaller90}
...
- [The Art of Data Visualisation with ggplot2: The TidyTuesday Cookbook](https://nrennie.rbind.io/art-of-viz/), by Nicola Rennie
...
We'll be using the **gt** and **gtsummary** packages, but there are many other good options.

Here's a good [overview](https://rfortherestofus.com/2019/11/how-to-make-beautiful-tables-in-r/) of many different packages for making tables in R.

Other interesting tables packages not mentioned in the above article, but worth exploring, are [**rtables**](https://insightsengineering.github.io/rtables/latest-tag/) and [**tinytable**](https://vincentarelbundock.github.io/tinytable/).
...
The {gt} package comes with several built-in datasets, including `exibble`, a [toy example tibble](https://gt.rstudio.com/reference/exibble.html).

Install the {gt} package, look at `exibble`, then create a table of the data using {gt}. 

How good can you get it looking? You can consult the [reference section on the {gt} website](https://gt.rstudio.com/reference/index.html) to see all the options available.
...
- [2025](https://posit.co/blog/2025-table-contest-winners/)

- [2024](https://posit.co/blog/2024-table-contest-winners/)
...
# Minute card feedback {.coral-bg}

What worked well today and what could be improved?

[Go to form](https://docs.google.com/forms/d/e/1FAIpQLSc4dEpLV6b2RwmOA5Q7Cq02sXdkQYw7IRSfIu_LHSpH6L3Qxw/viewform?usp=dialog){.larger200}
